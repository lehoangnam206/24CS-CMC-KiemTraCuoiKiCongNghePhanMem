-- Nap du lieu TECHBLUE vao database co san (MonsterASP.NET / SQL Server host chia se).
-- Khac data.sql: khong co USE [master] / CREATE DATABASE (tai khoan host khong co quyen),
-- khong CREATE TABLE (bang da do EnsureCreated() tao san), va cac khoi INSERT
-- duoc sap lai theo dung thu tu khoa ngoai.
-- Chay tren dung database cua ban (vi du db60757), KHONG chay tren master.

SET NOCOUNT ON;
GO

-- Xoa du lieu cu (bang rong hoac chi co tai khoan admin do app tu seed).
-- Thu tu xoa nguoc voi thu tu chen de khong vi pham khoa ngoai.
IF OBJECT_ID('[dbo].[PromotionProducts]', 'U') IS NOT NULL DELETE FROM [dbo].[PromotionProducts];
IF OBJECT_ID('[dbo].[ChatMessageLogs]', 'U') IS NOT NULL DELETE FROM [dbo].[ChatMessageLogs];
IF OBJECT_ID('[dbo].[ChatCustomerMemories]', 'U') IS NOT NULL DELETE FROM [dbo].[ChatCustomerMemories];
...
-- Kiem tra: so dong tung bang sau khi nap.
SELECT 'Categories' AS TableName, COUNT(*) AS [SoDong] FROM [dbo].[Categories]
UNION ALL SELECT 'Users', COUNT(*) FROM [dbo].[Users]
UNION ALL SELECT 'Products', COUNT(*) FROM [dbo].[Products]
UNION ALL SELECT 'ProductDetails', COUNT(*) FROM [dbo].[ProductDetails]
